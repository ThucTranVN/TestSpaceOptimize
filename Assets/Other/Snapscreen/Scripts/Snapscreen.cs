...
namespace NorthLab
{
    ...
    public class Snapscreen : MonoBehaviour
    {
        ...
        [SerializeField, Tooltip("Button name to take a screenshot from the Input settings"), Header("Input")]
        ...
        private IEnumerator TakeCropped(string path)
        {
            yield return new WaitForEndOfFrame();
            Texture2D capture = ScreenCapture.CaptureScreenshotAsTexture(factor);
            HandleImage(capture, path);

        }

        //Cropping and saving happens here
        private void HandleImage(Texture2D capture, string path)
        {
        ...
        private void Take()
        {
            ...
            {
                //Start coroutine to make sure Unity wait until the end of frame to take a screenshot
                StartCoroutine(TakeCropped(finalPath));
            }
            else
            {
                ScreenCapture.CaptureScreenshot(finalPath, factor);
            }

            //if GUI is hidden then unhide it
            if (hideGUI)
                Invoke("UnhideGUI", Time.deltaTime * unhideTime);
#endif
        }
